...
PBE v2.1 adds optional parity/completeness controls for projects where legacy parity, visual runtime behavior, or hardware-dependent behavior matters.

This profile does not replace Product, Project, Work, Test, Evidence, or Acceptance trees. It adds derived control views that make completion claims harder to overstate.
...
Do not force this profile onto small non-UI tasks, typo fixes, or ordinary backend changes unless the user asks for parity or surface completion control.
...
For legacy migration or parity-critical work, PBE must inventory visible or enabled legacy controls before claiming parity.

Inventory is not implementation scope by itself. Missing controls become Product, Work, Test, deferred, blocked, or out-of-scope nodes through normal PBE scope rules.
...
If a check cannot be run, record the reason and add not-runnable evidence instead of silently treating the check as passed.
...
If a parity or completion artifact reveals missing Product meaning, UX behavior, acceptance criteria, verification strategy, or accepted-work drift, create a Change Node and route through Impact Tree and the relevant human gate.
