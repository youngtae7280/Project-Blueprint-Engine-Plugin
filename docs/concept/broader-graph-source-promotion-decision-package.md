...
| Registry-backed `validate --all`        | `examples/internal-legacy/read-model-aggregate/read-model-slices.json` and local `pbe graph read-model validate --all`                                                                                           | Positive registry includes only Todo Search and Todo App PBE Run. It is non-enforcing Evidence, not source authority.                                        |
