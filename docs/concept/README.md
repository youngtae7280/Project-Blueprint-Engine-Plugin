...
Todo Search tree-native selected-slice artifacts are deprecated fallback/reference records, not source and not deleted;
Todo App is not-retirement-ready.
```
...
and records Todo Search as `retirement-candidate-not-deleted`, Todo App as `not-ready-structure-only`, and repo-wide
...
to link to explicit human decisions, and exposes the status through `graph read-model report-compiler-boundary --json`,
...
DevView Compliance Checker MVP Scope selects `scope-compliance-preview` as the first future checker axis. It defines the
...
The `graph read-model create-approved-proposal-state ...` command now creates or blocks an approved-state preview from a
...
boundaries after the controlled decision/readiness chain through approved apply dry-run, guarded apply reporting,
accepted Evidence, runtime Evidence satisfaction readiness, runtime-gated Equivalence readiness, and disabled Scope/CI
enforcement readiness. It is an audit summary only and does not add execution, approval automation, runtime Evidence
satisfaction, graph-source mutation, equivalence proof, scope enforcement, CI authority, required checks, branch
...
fixture coverage, hook install/trust, real LLM analyzer execution, current Todo graph apply/mutation, runtime Evidence
satisfaction, equivalence proof, and enforcement remain future work. It is a handoff summary only and creates no new
authority.

The DevView core baseline freeze report is generated by
`graph read-model report-devview-baseline --roadmap-audit <roadmapAudit> --final-handoff <finalHandoff> --json`. The
Todo App calibration outputs are
`examples/valid/todo-app-pbe-run/generated/devview-core-baseline-freeze.runtime-evidence-only.preview.json` and
`examples/valid/todo-app-pbe-run/generated/devview-core-baseline-freeze.runtime-evidence-only.preview.md`. The report
freezes completed, advisory, blocked, and future-only classifications for reviewers and workers. The refreshed baseline
includes approved apply dry-run, the blocked current graph-delta apply report, the Evidence decision record, the
accepted Evidence record, Runtime Evidence Satisfaction readiness blocked by the current obligation mismatch,
Equivalence readiness blocked by that runtime readiness, and Scope/CI readiness blocked/non-enforcing. Missing optional
chain/readiness artifacts produce warnings rather than traversal, apply, approval, runtime Evidence satisfaction,
equivalence, enforcement, or Project Memory extension authority.
...
The deterministic analyzer prompt/input contract pack is generated by
...
Its Todo App calibration outputs are
`examples/valid/todo-app-pbe-run/generated/ai-request-analyzer-pack.add-todo-runtime-evidence-only.preview.json` and
`examples/valid/todo-app-pbe-run/generated/ai-request-analyzer-pack.add-todo-runtime-evidence-only.preview.md`. The pack
is future analyzer guidance only; it does not call an LLM/API or generate a Request IR Candidate.

The analyzer command surface is exposed by
`graph read-model analyze-request --request <text> --pack <aiRequestAnalyzerPackPath> --json`. Without an explicit
`--external-candidate` or fully gated provider path, it reports `provider-disabled` and generates no candidate. With an
external candidate, it imports only candidate-only Request IR JSON and still requires deterministic validation before
traversal.

The AI Request Analyzer provider config boundary is previewed in
...
The boundary defines `disabled`, `configured-not-invoked`, `configured-invocation-enabled-preview`,
`configured-openai-invocation-enabled`, `unavailable`, `blocked-invalid-config`, and
`future-invocation-allowed-only-after-explicit-config` states. Provider config may record environment variable
references such as `OPENAI_API_KEY`, but it must not store, inspect, or print secret values; the live provider config
artifact itself still does not allow network calls or Request IR Candidate generation.

`configured-invocation-enabled-preview` is a mock-provider enablement candidate. It still keeps network/LLM/candidate
generation false unless it is paired with the deterministic mock response path. `--external-candidate` plus
`--invoke-provider` is blocked by policy.

`configured-openai-invocation-enabled` is OpenAI-live-specific config shape. Live invocation requires all runtime gates:
`--invoke-provider`, `--allow-network-provider`, and `--provider-mode openai`. The config artifact itself still keeps
`networkCallsAllowed`, `llmInvoked`, provider invocation, and candidate generation false.
...
Disabled, unavailable, configured-not-invoked, and future-invocation states remain blocked/import-required without an
external candidate, mock response path, or fully gated live OpenAI path. Unsafe or secret-looking provider config blocks
even external import.

The mock analyzer provider pipeline is implemented as a deterministic no-network safety harness. It requires
`--provider-config <invocationEnabledConfig> --invoke-provider --mock-provider-response <mockResponse>`, reads only a
tracked/local mock response artifact, and writes candidate-only Request IR output. The OpenAI path requires
`--provider-config <openAiLiveConfig> --invoke-provider --allow-network-provider --provider-mode openai`, reads the API
key value only from the configured environment variable after all gates pass, and still writes only candidate-only
Request IR output. `--external-candidate` plus `--invoke-provider` is blocked.

The clarification interview boundary and question-plan pack cover the alternate branch for ambiguous candidates. The
pack is generated by
`graph read-model generate-clarification-interview-pack --boundary <clarificationBoundaryPath> --candidate <candidatePath> --json`.
Its Todo App calibration outputs are
`examples/valid/todo-app-pbe-run/generated/clarification-interview-pack.add-todo-runtime-evidence-only.preview.json` and
`examples/valid/todo-app-pbe-run/generated/clarification-interview-pack.add-todo-runtime-evidence-only.preview.md`. The
pack is a question-plan preview only; it does not implement an interview UI, call an LLM/API, revise Request IR, run
...
It generates a plan only; it does not select nodes or edges as final scope, generate contract input, generate
instruction packs, mutate graph-source, apply graph deltas, approve work, or enforce scope.
...
trees in-place, and keeps only the current request, needed viewpoint buttons, and inspect actions visible before any
click. Mouse wheel and buttons use semantic zoom: graph distances change, but node boxes, labels, and edge strokes keep
stable screen size.
...
inspector and the graph selection banner. Detailed Project Memory, SubGraph, Instruction Source, node, edge, and tree
data is deferred to the right click inspector. Trees are not rendered as separate tree layouts; they are compact
clickable graph viewpoints whose node/edge chip lists appear only after click. The top history controls
