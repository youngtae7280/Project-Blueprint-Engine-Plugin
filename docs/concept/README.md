...
`graph read-model analyze-request --request <text> --pack <aiRequestAnalyzerPackPath> --json`, but provider execution is
disabled. Without an explicit `--external-candidate`, it reports `provider-disabled` and generates no candidate. With an
...
external candidate or mock response path. Explicit external import and the mock response parser/guard pipeline are the
current real-provider-free candidate-producing paths; unsafe or secret-looking provider config blocks even external
import.
...
tracked/local mock response artifact, and writes candidate-only Request IR output. Real OpenAI/API/LLM invocation remains
unimplemented, and `--external-candidate` plus `--invoke-provider` is blocked.
