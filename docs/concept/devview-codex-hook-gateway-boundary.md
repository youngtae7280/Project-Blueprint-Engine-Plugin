...
The event-level advisory report for a concrete `UserPromptSubmit` prompt is generated by:
...
The command emits `artifactRole: devview-user-prompt-submit-advisory-report`. When a preflight session chain reached
`instruction-pack-preview-generated-no-codex-execution`, it renders concise additionalContext Markdown with allowed
scope, forbidden scope/non-goals, evidence/output requirements, stop conditions, known risks, and validation-chain
summary. When preflight is missing or blocked, it writes a report-only status and the next deterministic
`run-preflight-session` command instead of running preflight itself. `off`, `guided`, and `strict-disabled` modes remain
non-enforcing: no hook install, no active session, no tool blocking, no Codex execution, no provider invocation, no graph
mutation/apply, no Evidence acceptance/satisfaction, no equivalence proof, and no scope/CI enforcement.

The Stop/Post Run advisory lifecycle report is generated by:

```text
graph read-model report-stop-post-run-advisory --user-prompt-advisory <userPromptSubmitAdvisory> --hook-health <healthReportOrBoundary> --json
```

The command emits `artifactRole: devview-stop-post-run-advisory-report`. It reads already-created artifacts such as the
preflight session chain, Instruction Pack, changed-file collection, advisory scope report, compact runtime report,
proposal-only Graph Delta preview, and Human Review Packet. Missing artifacts become `nextRequiredCommands`; the command
does not run Git, collect changed files, run `check-scope`, generate proposals, generate review packets, install hooks,
block tools, control Codex execution, revert files, mutate graph-source, apply deltas, approve work, accept Evidence,
prove equivalence, enforce scope, or configure CI.

For uncommitted post-run work, a changed-file collection artifact can be produced explicitly with:
...
These modes keep provenance separate: working-tree is tracked unstaged only, staged is staged index only, and untracked
uses untracked file paths only. They do not inspect hunks, revert or modify files, or enforce scope. The Stop/Post Run
advisory command may read any resulting `git-derived-changed-file-collection-preview` artifact through
`--changed-files`, but it still does not execute collection or scope checks automatically. If a provided changed-file
artifact records `collectionMode: staged-index` or `collectionMode: untracked-files`, the report points to the matching
`check-scope --staged` or `check-scope --untracked` follow-up command.

The first Hook Gateway script scaffold preview is generated by:
...
The command emits `artifactRole: devview-hook-script-scaffold-preview` with preview templates for `SessionStart`,
`UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop`. These are script role/template records only:
`hookScriptsImplemented: false`, `hookScriptsInstalled: false`, `hookGatewayActive: not-checked-preview-only`,
`installTrustDecisionImplemented: false`, `actualInstallOrTrustMutationImplemented: false`,
`strictModeEnabled: false`, `guidedEnforcementEnabled: false`, and `actualBlockingHookBehaviorImplemented: false` stay
fixed. Output authority guards reject active hook/config paths such as `.codex/hooks/...` and reject source artifact
overwrites before writing either JSON or Markdown.

The first materialized Hook Gateway script body preview is generated by:

```text
graph read-model generate-hook-script-templates --scaffold <hookScriptScaffold> --json
```

The command emits `artifactRole: devview-hook-script-template-preview` with review-only `powershell-preview` bodies for
`SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop`. The script bodies are not written to
`.codex/hooks` and are not active. They contain advisory-only statements such as reading a future
`DEVVIEW_USER_PROMPT_CONTEXT_PREVIEW` path, reminding the caller about scope, or suggesting report-only checks; they do
not run validation/traversal, execute Codex, mutate graph-source, apply graph deltas, approve work, satisfy Evidence,
prove equivalence, enforce scope, or configure CI.

The first Hook Gateway session manifest preview is generated by:
...
The command emits `artifactRole: devview-hook-session-manifest-preview` and bundles existing health, context, scaffold,
and template previews for a future session. The manifest remains `sessionStatus: not-started-preview-only`,
`hooksActive: false`, `hookScriptsInstalled: false`, `hookGatewayActive: not-checked-preview-only`, and
`bypassDetectionStatus: preview-only-non-enforcing`. It records transient session artifact candidates under
...
The first repo-local Hook Gateway script bundle preview is materialized by:

```text
graph read-model materialize-hook-script-bundle --script-templates <templatePreview> --session-manifest <sessionManifestPreview> --bundle-dir <bundleDir> --json
```

The command emits `artifactRole: devview-hook-script-bundle-preview` and writes advisory `.ps1` files to a dedicated
repo-local bundle directory such as `.tmp/devview-hook-script-bundle/...`. These files are review material only:
`hookScriptsImplemented: false`, `hookScriptsInstalled: false`, `hookGatewayActive: not-checked-preview-only`,
`activeHookSessionStarted: false`, `strictModeEnabled: false`, `guidedEnforcementEnabled: false`,
`preToolUseBlockingEnabled: false`, `scopeEnforced: false`, and `ciEnforcementEnabled: false` remain fixed. The command
rejects active hook/config targets such as `.codex/hooks`, generated source-authority outputs, and unsafe bundle
directories before writing scripts, JSON, or Markdown.
...
must avoid AI calls, network calls, full repo scans, patch/hunk inspection, file-content semantic analysis, and full
...
script bundle materializer, and activation-chain report commands are not currently included in the all-steps smoke
because adding session-manifest work pushed one local advisory snapshot over 5 seconds while leaving the core-critical
lane under budget. They remain lightweight and should be revisited when the smoke suite is split by lane or optimized.
These commands still do not install hooks, trust commands, block Codex execution, or enforce DevView.
