...
That working-tree mode is tracked unstaged only. It does not include staged or untracked files, inspect hunks, revert or
modify files, or enforce scope. The Stop/Post Run advisory command may read the resulting
`git-derived-changed-file-collection-preview` artifact through `--changed-files`, but it still does not execute the
collection or scope check automatically.
