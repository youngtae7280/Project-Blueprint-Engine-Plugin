...
optional frontend chain, hook activation chain, and readiness artifacts. It classifies the calibration state as
completed, advisory, blocked, or future-only so later workers do not confuse disabled readiness with completed
authority. Missing optional artifacts are reported as warnings and do not block baseline generation. The report does
not call an LLM, execute Codex, install hooks, mutate graph-source, apply graph deltas, automate approval, accept
Evidence, satisfy runtime Evidence, prove equivalence, enforce scope, configure CI, or grant Project Memory extension
authority.
