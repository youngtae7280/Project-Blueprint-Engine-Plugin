...
Task cards may appear in the ACEP compatibility package, but they are execution views of Cycle and Node Execution
Contracts. They do not replace Product, Project, Work, Test, Evidence, Acceptance, or execution-contract authority.
...
| `UX Audit`                    | UI/UX confirmation, WPD, VD, ACEP UI files when present           | Checks that confirmed UI/UX direction is represented in work, verification, task-card views, states, and evidence requirements.                                                                                                                                                                            | `ux-audit.md`                                                             | Automatic unless UI/UX coverage is missing                                 |
| `Generate ACEP`               | Blueprint artifacts, audits, execution strategy                   | Creates the Codex execution contract package: manifest, task-card views, traceability, UI/UX spec, validation commands, evidence checklist, final coverage check, and report template. Task-card views project contract obligations; they are not source authority.                                        | `.pbe/codex-execution-pack/`                                              | Automatic after audits pass                                                |
| `Run ACEP`                    | ACEP manifest, task-card views, traceability, UI/UX spec          | Executes selected and foundation scope only from the manifest and execution contracts. Runs validation, records evidence, respects parallel strategy, performs final coverage, and submits for review.                                                                                                     | Code changes, evidence, final report, review pack                         | Stops only on stop conditions                                              |
