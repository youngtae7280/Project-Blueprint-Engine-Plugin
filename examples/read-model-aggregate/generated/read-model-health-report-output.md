...
| Dry-run compiler input                      | `compiler-input-dry-run-pass`; `change-todo-search-whitespace-normalization-dogfood`; 2 graph artifacts / 3 policies / 2 evidence entries / 2 scope candidates / 4 output sources / 2 stop sources                                                  |
...
| Generated vs hand-written contract diff     | `contract-diff-detected`; `non-blocking-review-diff`; `compiler-equivalence-not-proven`; 5 differing fields; `examples/read-model-aggregate/generated/execution-contract-dry-run.diff.json`                                                         |
...
| Source authority gap preview                | `contract-source-authority-gap-preview-pass`; 1 remaining losses (1 semantic / 0 policy); fields allowedScope, knownRisks; next `risk-source-authority`; `examples/read-model-aggregate/generated/contract-source-authority-gap.preview.json`       |
| Contract semantic diff review               | `compiler-promotion-not-ready`; severity `high`; unknown semantic diffs 0; unknown fields none; conservative-restriction: 1, safe-additive: 2, semantic-loss: 1, metadata-only: 2                                                                   |
