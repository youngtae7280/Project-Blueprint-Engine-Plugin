...
Autoflow data is stored in `pbe-state.json.autoflow`. Supported state transitions must be written through the `pbe` CLI, not by hand-editing the state file. Autoflow tracks:
