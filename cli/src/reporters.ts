...
Commands:
  ...
  benchmark compare-code-graph-parity
                       Compare a DevView code subgraph against raw Graphify graph.json counts without running Graphify
  ...
  graph import-graphify-code-subgraph
                       Convert a static Graphify export into a validated DevView code subgraph source fact
  graph extract-code-subgraph
                       Extract JS/TS/C# file, symbol, import, and call facts into a validated code subgraph
  graph plan-code-subgraph-merge
                       Plan code subgraph additions to the unified Maintainability Graph without mutation
  graph plan-code-subgraph-refresh
                       Plan stale code subgraph candidates from explicit changed files without extraction or mutation
  graph derive-code-symbol-links
                       Derive maintenance-to-code symbol link source facts from code subgraph plus graph data without mutation
  graph query-unified
                       Query neighbors, paths, or explain context over supplied unified graph source facts
  graph report-code-impact
                       Report callers, callees, import dependents, containment, and maintenance impact from a static code subgraph
  graph validate-code-symbol-links
                       Validate maintenance-to-code symbol link source facts without graph-source mutation
  graph validate-code-subgraph
                       Validate a static DevView code subgraph source fact without extraction or graph-source mutation
  ...
  security report-ci-branch-activation-authority-readiness
                       Report signed policy/RBAC/provider prerequisites before CI/branch activation can be authoritative
  ...
  graph render-code-graph-html
                       Render a read-only HTML inspector from a validated DevView code subgraph
...
Options:
  ...
  --include <patterns> Include path patterns for graph extract-code-subgraph, repeatable or comma-separated.
  ...
  --code-subgraph <file>
                       Static DevView code subgraph source fact for benchmark compare-code-graph-parity, graph validate-code-subgraph, render-code-graph-html, plan-code-subgraph-merge, plan-code-subgraph-refresh, derive-code-symbol-links, report-code-impact, query-unified, or View Tree symbol selection.
  --code-subgraph-validation <file>
                       Passed DevView code subgraph validation report for graph render-code-graph-html, plan-code-subgraph-merge, or graph validate-code-symbol-links.
  ...
  --code-symbol-links-validation <file>
                       Passed code symbol link validation report for graph report-code-impact, query-unified, or graph read-model generate-view-tree symbol selection.
  --changed-symbol <id>
                       Changed code node id for graph report-code-impact. May be repeated or comma-separated.
  --changed-file <path>
                       Changed source file path for graph plan-code-subgraph-refresh. May be repeated or comma-separated.
  --mode <mode>        Query mode for graph query-unified: neighbors, path, or explain.
  --node <id>          Node id for graph query-unified neighbors or explain mode.
  --source-node <id>   Source node id for graph query-unified path mode. --from is also accepted.
  --target-node <id>   Target node id for graph query-unified path mode. --to is also accepted.
  --max-depth <n>      Maximum directed BFS depth for graph query-unified path mode. Defaults to 4.
  ...
  --validation-output <file>
                       Validation output for graph import-graphify-code-subgraph, graph extract-code-subgraph, graph read-model run-clarification-chain, or post-mutation graph read-model apply-graph-delta/apply-guarded-graph-update.
  ...
  --graphify-export <file>
                       Static Graphify export fixture for benchmark compare-code-graph-parity, benchmark validate-graphify-import, or graph import-graphify-code-subgraph.
  --graphify <file>   Alias for --graphify-export on benchmark compare-code-graph-parity or graph import-graphify-code-subgraph.
